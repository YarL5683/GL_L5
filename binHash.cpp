...
=======
void BinHash::Add(std::shared_ptr<node>& leaf, const std::string& inp_str,
                  int hash_str) {
  if (leaf != nullptr) {
    if (hash_str > leaf->hash_data) {
      Add(leaf->right, inp_str, hash_str);
      ...
      Add(leaf->left, inp_str, hash_str);
>>>>>>> 09caefbfe94d8f54d5a11cb3d648cde9e29fa8ea
    }
...
bool BinHash::Find(const std::shared_ptr<node>& current, int inp_hash, const std::string& word)
{
    ...
    return false;
=======
bool BinHash::Find(const std::shared_ptr<node>& current, int inp_hash,
